import { Component } from '@angular/core';
import { DataService } from '../data.service';
@Component({
  selector: 'app-card10v2',
  templateUrl: './card10v2.component.html',
  styleUrls: ['./card10v2.component.css']
})
export class Card10v2Component {

  selectedMatiere: string | null = null;

  selectRadioMatiere(option: string): void {
    this.selectedMatiere = option;
    this.updateData();
  }
  constructor( private dataService: DataService){}
  updateData() {
    // Obtenir les données existantes du service
    const existingData = this.dataService.getData();
    const newData = {
      ...existingData,
      Matiere:this.selectedMatiere,
     
      
    };
    // Mettre à jour les données dans le service
    this.dataService.updateData(newData);
    console.log(this.dataService.getData());
  }
}
